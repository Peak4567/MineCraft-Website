<?php
		if(isset($_POST['settings_web_submit']))
		{
			...
			$file = $dir . basename($_FILES['icon']['name']); 
			$file_for_index = $for_index. basename($_FILES['icon']['name']);
			move_uploaded_file($_FILES['icon']['tmp_name'], $file);
			
			$file_ = $dir . basename($_FILES['bg']['name']); 
			$file_for_index_ = $for_index. basename($_FILES['bg']['name']);
			move_uploaded_file($_FILES['bg']['tmp_name'], $file_);
	
			
	
			
			$sql_edit_tmtopup = 'UPDATE setting SET backend_password = "'.$_POST['backend_password'].'", name_server = "'.$_POST['name_server'].'", ip_server = "'.$_POST['ip_server'].'", port_server = "'.$_POST['port_server'].'", version_server = "'.$_POST['version_server'].'", detail_server = "'.$_POST['detail_server'].'", max_reg = "'.$_POST['max_reg'].'", query_port = "'.$_POST['query_port'].'", bg = "'.$file_for_index_.'", slash = "'.$_POST['slash'].'", icon = "'.$file_for_index.'", page_facebook = "'.$_POST['page_facebook'].'", www = "'.$_POST['url'].'"';
...
?>
		...
		<div class="card-body">	
			<form name="settings_web_submit" method="POST" enctype="multipart/form-data">
				<div class="row">
			        ...
			        <div class="col-md-6 mb-3">
			            <label for="bg">พื้นหลัง</label>
                                    <input type="file" class="form-control" id="bg" name="bg" required="">
			        </div>
                    <div class="col-md-6 mb-3">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="url">URL WebShop <font color="red"> * ใช้สำหรับเชื่อม API TRUEWALLET(แก้ไขตรงIP)</font></label>
					...
					<hr class="is-divider">
                                        <div class="text-center" width="24" data-toggle="collapse" href="#collapseDetail-112" role="button" aria-expanded="false" aria-controls="collapseDetail-112"><i class="fa fa-arrow-circle-down"></i> อัพเดทเว็บไซต์</div>
...
<div>UPDATE WEBSHOP<a target="_blank" class="foot_hover"  href="?page=backend&menu=update"> CLICK</a><br></div></div>
